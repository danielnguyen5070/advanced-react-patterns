...
type CheckboxType = { bChecked: boolean }
type CheckboxAction = { type: 'toggle' } | { type: 'reset', initialValue: boolean }

function checkboxReducer(state: CheckboxType, action: CheckboxAction): CheckboxType {
    switch (action.type) {
        case 'toggle':
            return { bChecked: !state.bChecked };
        case 'reset':
            return { bChecked: action.initialValue };
        default:
            throw new Error();
    }
}

function useCheckbox(initialValue: boolean) {
    const initRef = React.useRef(initialValue);
    initialValue = initRef.current;

    const [check, dispatch] = React.useReducer(checkboxReducer, { bChecked: initialValue })
    const { bChecked } = check;
    ...
    return { bChecked, setChecked, reset };
}
...
export function SportDataView({ sport }: { sport: SportData }) {
    ...
    const { bChecked, setChecked, reset } = useCheckbox(defaultTricks);
    return (
            ...
            <div className="flex space-x-12 items-start">
                <div className="flex flex-col">
                    ...
                    <div className="items-center space-x-2 justify-center flex">
                        <input checked={bChecked} type="checkbox" id="toggleTricks" className="w-4 h-4" onClick={() => setChecked()} />
                        <label htmlFor="toggleTricks" className="cursor-pointer text-blue-600 underline">Show/Hide Tricks</label>
                    </div>
                    <button onClick={reset} className="mt-2 px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition" >Reset</button>
                </div>
                <div className={`${bChecked ? "block" : "hidden"}`}>
                    {/* Sport Name */}
                    <section>
